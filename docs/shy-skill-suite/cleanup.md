...
| CL-0005 | `deadcheck:req0039/0041/0043` | 7 条 check 注册给 out-of-scope REQ（`req0039-*`、`req0041-*`、`req0043-*`），永不执行 | iteration-3 | open | | `run_checks.py --list` 含它们；对应 REQ 均 out-of-scope |
...
| CL-0025 | `readout:B-合并` | 读数 B 只打印 `cleanup_open`（一来源）；fix 工单数、红不变量数未并入 → "有多少问题"仍需手拼 | iteration-3 | open | | 设计 §4.5 |
