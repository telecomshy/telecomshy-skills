...
1. 新增技能内 `commands/` 目录（5 个可复制模板，body 指向对应分支）：
...
- [x] `commands/` 下 5 个模板存在（`shy-grill/review/eval/apply/next.md`），各含 `description` 且 body 引用技能内正确相对路径（`references/*.md`、`scripts/*.py`）。
...
- [x] `lifecycle.md` 的「斜杠快捷」小节列出 5 个命令与对应分支；`SKILL.md` 资源清单含 `commands/`。
