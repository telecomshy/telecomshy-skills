...
- `commands/` — opencode slash-command templates (`shy-start` / `shy-grill` / `shy-review` / `shy-eval` / `shy-reqs`); copy them to `~/.config/opencode/commands/` to take effect, see "Slash shortcuts" in `references/lifecycle.md` for details. `/shy-start [<skill>]` is the **start-a-new-skill** entry point (grill → write requirements → scaffold, one flow, skill name optional); `/shy-reqs <skill>` is the **single-skill** REQ report (grouped by kind → status, collapsible).
