...
- `commands/` — opencode slash-command templates (`shy-skill-start` / `shy-skill-grill` / `shy-skill-review` / `shy-skill-eval` / `shy-skill-reqs`); copy them to `~/.config/opencode/commands/` to take effect, see "Slash shortcuts" in `references/lifecycle.md` for details. `/shy-skill-start [<skill>]` is the **start-a-new-skill** entry point (grill → write requirements → scaffold, one flow, skill name optional); `/shy-skill-reqs <skill>` is the **single-skill** REQ report (grouped by kind → status, collapsible).
