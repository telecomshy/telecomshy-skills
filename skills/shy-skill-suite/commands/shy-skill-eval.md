...
调用 `skill` 工具加载 `shy-skill-suite` 技能，然后按 `references/running-evals.md` 的「独立评测路径」执行。**只取证、不改技能、不提修复建议**——修复走 `/shy-skill-review` → 分拣 → 用户确认后实施。

参数：`$ARGUMENTS` 形如 `<skill> [触发|有效性|两者]`，轴默认**两者**。**技能名必填**：`$ARGUMENTS` 未给技能名时**直接询问用户**要评测哪个技能（不默认任何一个）。
...
7. **与 `/shy-skill-review` 互不调用**：本命令不产 findings、不分拣；复审另由用户触发。
