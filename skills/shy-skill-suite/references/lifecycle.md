...
把模糊想法逼成明确需求再落盘。用**设计树 + frontier** 的机制：每个决策分出下级决策；只问"前置已定"的那批（frontier），一轮问完 frontier，**每问附上你的建议答案**，等用户回答后再算下一轮。**事实自己查**（读仓库、跑命令、派子 agent），只把**决策**留给用户。
...
frontier 空 = 逼问完成，且用户确认理解一致，才进入下一步。

**可跳过**：当需求已明确——用户已给出目标、触发说法与验收证据，或这是某份既有 REQ 的下一轮迭代——就跳过逼问直接进入阶段 1；跳过时在 REQ 的「备注 / 待办」记一句 `未逼问，依据：…`。改动微小且无歧义时尤其适合跳过。
