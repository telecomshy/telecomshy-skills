...
**完成判据**：`validate_skill.py <skill_dir>` → `status: ok`；该增量声明的每条验收标准都能指出证据（命令输出 / 文件 / 计数），无一条标「待验证」；**且已进入阶段 3——已产出 `findings.json` 并呈现**（可为空）。**实现完成 ≠ 改完代码，而是"过了校验 + 审过一轮"。**

### 3 · 复审

按 `reviewing-skills.md` 执行（行为轴：触发 + 有效性；需求轴：需求一致性 Spec——**先跑 `run_checks.py`，只有 `（语义）` 项才逐条读**；标准轴：结构 / 脚本 / 安全），用 `running-evals.md` 的脚本取证据。产出**分轴**、带优先级的 findings，并落盘 `findings.json`（schema 见 `reviewing-skills.md`）。

**按改动规模分层取证**（省钱、且不循环）：

1. **任何改动（默认）**：跑**秒级门**——`validate_skill.py` + `run_checks.py` + `selftest.py`；据此产出 `findings.json`（**可为空 = 可合入**）。
2. **行为相关改动 / 里程碑**：**再加** Step 1 / Step 2 的行为 eval（触发率、with/baseline delta，分钟级）。
3. **轻量门不能替代行为轴**：改了触发 / 行为，必须跑行为 eval。
4. **跳过**：用户明确说"别审 / 跳过复审"时可不跑，但须在结论里注明"**未做复审**"（不静默豁免）。

> 注意：本步强制的是**取证**，不是"修到过"。findings 可以为空；**回写按下面「呈现门禁」分两种**——机械记账自动、判断项停下——所以不会自动进入审→修→审 的循环。

**呈现门禁（阶段 3 → 4）——停不停，只看有没有需要人拍板的事**：

- **无 P0/P1、无新发现** → **不停**：机械记账**自动回写**（见阶段 4），继续下一轮。**不必出 HTML 报告**（一句话汇报即可）。
- **有 P0/P1、或发现新事项**（要新开 REQ / 改需求语义） → **停下**：产出 `findings.json` + **HTML 报告并打开**（`running-evals.md`），交人拍板；此时**未**修改任何 REQ、**未**改动技能文件。

**HTML 报告的时机**：**有 findings（P0/P1 或行为轴结论）时才出**；纯秒级门无发现不出——避免每改一行就弹浏览器。

### 4 · 回写需求（本环的关键，skill-forge 缺）

**机械记账不需确认**：秒级门绿、无 P0/P1、无新事项时，**直接回写**（勾验收 / 记迭代 / 改 `status`），不必停下等人。**涉及判断时才需确认**（有 P0/P1 不能置 `done`、发现新事项要新开 REQ、需求语义要变——见阶段 3 呈现门禁）。回写内容：
...
- `run_checks.py` 全部 `check:` 项通过，`（行为）` / `（语义）` 项判定完毕（**本轮 diff 触碰的 REQ（`deferred` / `out-of-scope` 除外）无未标条目**；全局未标只记数、不阻断）；
