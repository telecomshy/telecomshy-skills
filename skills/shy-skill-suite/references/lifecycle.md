...
| `/shy-review` | 触发**评审路**：三轴 + 评测，出报告供逐条分拣（`reviewing-skills.md` + `running-evals.md`） |
| `/shy-apply` | 按分拣实施「立即修」的项（阶段 4） |
| `/shy-next` | 列 REQ frontier（`track_requirements.py`） |

`/shy-review` 已含评测——eval 是评审 Step 1/2 的执行层（见 `running-evals.md`），故不单列 `/shy-eval`。**实现路径不会自动进入评审。**
...
**完成判据**：`validate_skill.py <skill_dir>` → `status: ok`；该增量每条验收标准都能指出证据（命令输出 / 文件 / 计数），无一条标「待验证」；**静默秒级门通过**；**做减法前已查引用**——删 / 隐藏对外呈现或检查（按钮 / 字段 / 显示 / `check`）前，已检索技能文件与 `run_checks.py` 注册表、确认无规则 / 检查仍要求它；**改行为的涟漪已付清**——受影响的契约文本（旧 REQ 的验收 / 技能文档）已当轮订正，新行为已有回归检查。**实现完成 = 交付合格，不要求"审过一轮"。**
...
**行为相关 / 里程碑**：加 Step 1 / Step 2 的行为 eval（触发率、with/baseline delta）——评审路里唯一"分钟级"的部分，只在真需要时跑；日常轻改动不必。

**评审收尾 = 报告 + 逐条分拣**（这是评审路的终点）：

1. 产出 `findings.json` + **HTML 报告并打开**（`running-evals.md`）。
2. **逐条**请用户分拣（**不做批量**）——`--serve` 报告页内逐条勾选（默认立即修），末尾点「提交给 agent」；静态报告只读、不能勾选；也可直接在对话里答：
   - **立即修** → 在本轮实施（见阶段 4）。
   - **以后修** → 落盘成 `ready` 的 REQ（进 frontier）。
   - **丢弃** → **不落盘、不留痕**。
3. 按用户所选**实施一次**，再跑静默秒级门，**结束**——**不自动再审、不自动进入下一轮**。
...
需求是**活文档**：用户确认落盘后，每轮评审按用户的分拣回写它（阶段 3 → 4）。**实现与评审两路分离**——实现完停下、评审由用户触发；技能与需求一起演进，而不是需求写完就冻结。
