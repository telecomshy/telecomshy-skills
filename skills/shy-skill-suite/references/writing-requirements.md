...
- `title` 规范：**面向人的白话一句话**，说清这个 REQ 做什么；必要的技术词可保留，但以人话为主。它同时是 `/shy-skill-reqs` 报告里直接可见的说明（报告不另设 `plain`/`summary` 字段）。
...
6. **## 验收标准** — 可勾选、可独立验证的清单（`- [ ] ...`），**必须端到端可验**。每条按**怎么判定**标四类之一（见 §4：`` `check: <name>` `` / **`（行为）`** / **`（语义）`** / **`（episode）`**）：静态可判 → `` `check: <name>` ``；必须真跑 agent 才知道 → **`（行为）`**（进 Step 2 的 with/baseline 对照）；只能语义判断 → **`（语义）`**（写明由谁、依据什么判定）；一次性事实 → **`（episode）`**（冻结、不复扫）。
7. **## 范围外** — 明确不做的事，防止范围蔓延。
