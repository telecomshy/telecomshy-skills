...
description: 技能开发套件：面向「技能」这一产物的开发全流程操作规范。当你在新建或修改一个技能、为技能写需求文档（REQ-NNNN 命名 / 结构 / 验收标准）、复审某个技能（触发可靠性、行为有效性、结构预算、脚本与安全）、写或改技能的 SKILL.md、校验技能结构、或推进技能开发的迭代闭环并回写需求时使用。
metadata:
  name_cn: 技能开发套件
