...
三探索对拍按子代理数量消耗 token（3 个探索者 = **3 个子代理**）。**每次运行、委派前**先报规模并确认去向：

- 用户本轮已明确要求「双/三探索对拍 / 要质量」→ 视为已确认，报一句规模即执行；
- 否则**默认走原生轻跑**（按上游 `improve-codebase-architecture` 原样：单个探索子代理），要用户主动选「三探索对拍」才升级。
